...
export class MapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  marker: any;
  @Input() update = false;
  @Input() center = { lat:43.9485185 , lng:-79.0184377 };
  ...
  async initMap() {
    try {
      ...
      console.log(e);
      this.center = { lat: 43.9485185, lng:-79.0184377 };
    ...
    try {
      ...
      this.map = new googleMaps.Map(mapEl, {
        ...
        mapTypeControlOptions: {
          mapTypeIds: [googleMaps.MapTypeId.ROADMAP, 'Food-delivery']
        }
      });
      var mapType = new googleMaps.StyledMapType(style, { name: 'Grayscale' });
      this.map.mapTypes.set('Food-delivery', mapType);
      this.map.setMapTypeId('Food-delivery');
